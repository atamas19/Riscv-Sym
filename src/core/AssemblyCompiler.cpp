...
uint32_t AssemblyCompiler::getInstruction(const std::string& instructionString)
{
    static const std::unordered_map<std::string, std::function<uint32_t(const AssemblyInstruction&)>> instructionMap = {
        ...
        {"and",  [this](const AssemblyInstruction& ins) { return assembleAND(ins);  }},
            // RV32M instructions
        {"mul",   [this](const AssemblyInstruction& ins) { return assembleMUL(ins);   }},
        {"mulh",  [this](const AssemblyInstruction& ins) { return assembleMULH(ins);  }},
        {"mulhsu",[this](const AssemblyInstruction& ins) { return assembleMULHSU(ins);}},
        {"mulhu", [this](const AssemblyInstruction& ins) { return assembleMULHU(ins); }},
        {"div",   [this](const AssemblyInstruction& ins) { return assembleDIV(ins);   }},
